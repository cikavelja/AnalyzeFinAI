...
def search_todos() -> List[str]:
    """Search for TODO/FIXME/HACK comments in production code (test dirs excluded)."""
    todos = []
    patterns = ["TODO", "FIXME", "HACK"]
    exclude_test_dirs = {"test", "tests", "__tests__", "spec", "__mocks__", "fixtures"}

    try:
        for root, dirs, files in os.walk(Path.cwd()):
            dirs[:] = [d for d in dirs if d not in EXCLUDE_DIRS and d not in exclude_test_dirs]
